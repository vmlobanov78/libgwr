...
guint32     IExtensibleHeader::s_button_width      =   20;
guint32     IExtensibleHeader::s_button_height     =   15;
//  ############################################################################
//
//  IExtensibleHeader
//
    ...
    : IExtensibleHeader()
{
    ...
    //  cf gtk bug in LogTune::View::nsp_add(LogTune::Nsp* _Nsp)
    #if GWR_GTK_VERSION_GE(3,0)
    gtk_widget_set_vexpand( d_vbox, FALSE );
    #endif

    gtk_widget_set_size_request( d_button   ,                                   // reduce minimal size of button
        IExtensibleHeader::s_button_width   ,
        IExtensibleHeader::s_button_height  );

    gwrgtk_button_set_label_font( d_button, "Monospace", PANGO_STYLE_NORMAL, PANGO_WEIGHT_NORMAL, 8 );
...
void        ExtensibleHeader::show()
{
    ...
    gtk_label_set_text( GTK_LABEL(gtk_bin_get_child(GTK_BIN(d_button))), "~" );
    ...
    : IExtensibleHeader()
{
    ...
    d_vbox                      = gwrgtk_vbox_new(FALSE,0);
        d_frame                 = gtk_frame_new( "" );
            d_hbox              = gwrgtk_hbox_new(FALSE,3);
                d_button        = gtk_button_new_with_label("?");
                d_label         = gtk_label_new( _name);
            a_child             = NULL;
